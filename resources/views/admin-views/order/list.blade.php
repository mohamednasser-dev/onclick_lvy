            ...
            <div class="row align-items-center mb-3">
                ...
                <div class="col-3">
                    ...
                    <select class="custom-select custom-select-sm" name="branch"
                            onchange="filter_Users_orders(this.value)">
                        <option disabled>--- {{trans('messages.select')}} {{trans('messages.user')}} ---</option>
                        <option value="0" {{session('user_filter')==0?'selected':''}}>{{trans('messages.all')}} {{trans('messages.user')}}</option>
                        @foreach(\App\User::all() as $user)
                            <option value="{{$user['id']}}" {{session('user_filter') == $user['id']?'selected':''}}>{{$user['name']}}</option>
                        @endforeach
                    </select>
                    <!-- End Select -->
                </div>
                ...
                <div class="row justify-content-between align-items-center flex-grow-1">
                    ...
                    <div class="col-lg-5 mb-3 mb-lg-0 d-sm-flex pr-0 ">
                      ...
                      <div class="cl-lg-3  ml-3">
                        <select class="custom-select custom-select user" id="user" name="timeSlot">
                            <option disabled>--- {{trans('messages.select')}} {{trans('messages.user')}} ---</option>
                            @foreach(\App\User::all() as $user)
                                    ...
                                    <div class="card card-sm">
                                        <div class="card-body">
                                            ...
                                            <div class="d-flex justify-content-between align-items-center mb-3">
                                                ...
                                                <!-- Checkbox Switch -->
                                                <label class="toggle-switch toggle-switch-sm" for="toggleColumn_timeSlot">
                                                    <input type="checkbox" class="toggle-switch-input"
                                                           id="toggleColumn_timeSlot" checked>
                                                    <span class="toggle-switch-label">
                                                    <span class="toggle-switch-indicator"></span>
                                                  </span>
                                                </label>
